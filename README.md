...
// 関数 funcName() 実行
LPtr<LThreadObj> thread = vm->new_Thread() ;
...
C++ 言語とやり取りをする場合いくつかの方法があり、最も簡単な方法が DLL エクスポート関数をインポートする形で呼び出すことです。  
数値やポインタ、構造体ポインタを受け取り、オブジェクトを返却しない関数の場合十分な方法です。

もう一つが NativeObject を利用する方法です。  
